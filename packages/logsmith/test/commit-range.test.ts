...
import { getCommits, getLatestTag, execGit } from '../src/utils'
...
describe('commit range functionality', () => {
  it('should find commits between two tags', () => {
    ...
    console.log('First few commits:', commits.slice(0, 3).map(c => ({ hash: c.hash, message: c.message })))
  ...
  it('should handle non-existent tags gracefully', () => {
    ...
    console.log('Commits with non-existent from tag:', commits.length)
